...
function dia_chordConstructor(key,degree,interval,number)
{
  chordtones = [];
  currentdegree = degree;
  notes = getScale(key,'TTSTTTS');
...
function playNotes(notearray, oscillators, wave, context)
{
  ...
  for (i = 0; i < notearray.length; i++)
  {
    ...
    oscillators[i].start();
    oscillators[i].stop(context.currentTime + 1.5);
...
function play(Note){
  ...
  chordscale = getScale(chordkey,MajorScale);

  notedegree = chordscale.indexOf(notename.slice(0,-1)) +1;
  chordnotes = dia_chordConstructor(chordkey,notedegree,constructinterval,numberofnotes);
  ...
  //
  // PianoOscilators[slot+1] = context.createOscillator();
  // PianoOscilators[slot+1].type = "sine";
  // PianoOscilators[slot+1].frequency.value = notetoFreq();
  // PianoOscilators[slot+1].connect(context.destination)
  // PianoOscilators[slot+1].start();
  // PianoOscilators[slot+1].stop(context.currentTime + 1);
  //
  //
  //
  //
  // PianoOscilators[slot+2] = context.createOscillator();
  // PianoOscilators[slot+2].type = "sine";
  // PianoOscilators[slot+2].frequency.value = notetoFreq(single_transpose(notename,7));
  // PianoOscilators[slot+2].connect(context.destination)
  // PianoOscilators[slot+2].start();
  // PianoOscilators[slot+2].stop(context.currentTime + 1);




  slot = (slot+3)%PianoOscilators.length;
